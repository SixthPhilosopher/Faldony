package com.kiss.backend.controller

import com.kiss.backend.model.dto.*
import com.kiss.backend.model.entity.Person
import com.kiss.backend.repository.PersonRepository
import io.swagger.v3.oas.annotations.Operation
import io.swagger.v3.oas.annotations.media.Content
import io.swagger.v3.oas.annotations.media.Schema
import io.swagger.v3.oas.annotations.responses.ApiResponse
import io.swagger.v3.oas.annotations.tags.Tag
import jakarta.validation.Valid
import org.springframework.http.HttpStatus
import org.springframework.http.ResponseEntity
import org.springframework.web.bind.annotation.*
import org.springframework.web.server.ResponseStatusException

@RestController
@RequestMapping("/api/v1/parties")
@Tag(name = "Parties", description = "Party (person) management endpoints")
class PartyController(
    private val personRepository: PersonRepository
) {

    /** Full list (reference data — plain, unbounded; used for dropdowns too). */
    @GetMapping
    @Operation(summary = "List all parties")
    @ApiResponse(responseCode = "200", description = "List of parties")
    fun getParties(): List<PersonDto> =
        personRepository.findAll().map { toDto(it) }

    @GetMapping("/{id}")
    @Operation(summary = "Get party by ID")
    @ApiResponse(responseCode = "200", description = "Party details")
    @ApiResponse(
        responseCode = "404",
        description = "Party not found",
        content = [Content(schema = Schema(implementation = ApiError::class))]
    )
    fun getPartyById(@PathVariable id: Long): ResponseEntity<PersonDto> {
        val person = personRepository.findById(id)
            .orElseThrow { ResponseStatusException(HttpStatus.NOT_FOUND, "Party not found") }
        return ResponseEntity.ok(toDto(person))
    }

    @PostMapping
    @Operation(summary = "Create party")
    @ApiResponse(responseCode = "201", description = "Party created")
    fun createParty(@Valid @RequestBody req: PersonCreateRequest): ResponseEntity<PersonDto> {
        val person = Person(name = req.name, email = req.emails)
        val saved = personRepository.save(person)
        return ResponseEntity.status(HttpStatus.CREATED).body(toDto(saved))
    }

    @PatchMapping("/{id}")
    @Operation(summary = "Update party")
    @ApiResponse(responseCode = "200", description = "Party updated")
    @ApiResponse(
        responseCode = "404",
        description = "Party not found",
        content = [Content(schema = Schema(implementation = ApiError::class))]
    )
    fun patchParty(
        @PathVariable id: Long,
        @Valid @RequestBody req: PersonPatchRequest
    ): ResponseEntity<PersonDto> {
        val person = personRepository.findById(id)
            .orElseThrow { ResponseStatusException(HttpStatus.NOT_FOUND, "Party not found") }

        req.name?.let { person.name = it }
        req.emails?.let { person.email = it.toMutableSet() }

        val saved = personRepository.save(person)
        return ResponseEntity.ok(toDto(saved))
    }

    @DeleteMapping("/{id}")
    @Operation(summary = "Delete party")
    @ApiResponse(responseCode = "204", description = "Party deleted")
    @ApiResponse(
        responseCode = "404",
        description = "Party not found",
        content = [Content(schema = Schema(implementation = ApiError::class))]
    )
    fun deleteParty(@PathVariable id: Long): ResponseEntity<Void> {
        val person = personRepository.findById(id)
            .orElseThrow { ResponseStatusException(HttpStatus.NOT_FOUND, "Party not found") }
        personRepository.delete(person)
        return ResponseEntity.noContent().build()
    }

    private fun toDto(person: Person) = PersonDto(
        id = person.id!!,
        name = person.name,
        emails = person.email
    )
}